...
from CNNs.CNN2D import TESTEtAl
from CNNs.CNN3D import C3DEtAl
from CNNs.hybridsn import HybridEtAl

from Transformers.DCTN import DCTN, EISA
from Transformers.hit import HiT, ConvPermuteMLP
from Transformers.SSFTTnet import SSFTTnet

from Mambas.MambaHSI import MambaHSI
# from Mambas.HyperMamba import VSSM


from Ours.BRWKVHSI import BinaryRWKVHSI
from Ours.SSLANet import SSLANet
...
def get_model(name, **kwargs):
    ...
    if name == "nn":
        ...
        model = BinaryRWKVHSI(in_channels=n_bands,
        ...
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = SSLANet(in_channels=n_bands, num_classes=n_classes, input_size=kwargs["patch_size"])
        lr = kwargs.setdefault("learning_rate", 0.001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        kwargs.setdefault("batch_size", 100)
        kwargs.setdefault("test_stride", 1)
    elif name == "svd":
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = SVDMamba(input_channels=n_bands, num_classes=n_classes, num_layers=2, d_model=64, d_state=16, use_pruning=True)
        lr = kwargs.setdefault("learning_rate", 0.001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
    ...
    #     # default of  0.002 to  train the  network"
    #     lr = kwargs.setdefault('lr', 0.001)
    #     model = FFTNet2D(n_bands, n_classes, 32, 4,4)
    #     # For Adadelta, we need to load the model on GPU before creating the optimizer
    #     model = model.to(device)
    #     optimizer = optim.AdamW(model.parameters(), lr=lr, weight_decay=1e-4)
    #     criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
    ...
    #     # default of  0.002 to  train the  network"
    #     lr = kwargs.setdefault('lr', 0.001)
    #     model = HyperspectralTransformer(num_classes=n_classes, nbands=n_bands, dim=128, depth=3, heads=4,
    #                                  use_decay_attention=True)
    #     # For Adadelta, we need to load the model on GPU before creating the optimizer
    #     model = model.to(device)
    #     optimizer = optim.AdamW(model.parameters(), lr=lr, weight_decay=1e-4)
    #     criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
    ...
    #     # default of  0.002 to  train the  network"
    #     lr = kwargs.setdefault('lr', 0.001)
    #     model = SSMN(n_bands=n_bands, num_classes=n_classes)
    #     # For Adadelta, we need to load the model on GPU before creating the optimizer
    #     model = model.to(device)
    #     optimizer = optim.AdamW(model.parameters(), lr=lr, weight_decay=1e-4)
    #     criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
    ...
    #     kwargs.setdefault("batch_size", 100)
        ...
        # default of  0.002 to  train the  network"
        lr = kwargs.setdefault('lr', 0.001)
        model = MambaHSI(in_channels=n_bands, num_classes=n_classes,)
        # For Adadelta, we need to load the model on GPU before creating the optimizer
        model = model.to(device)
    ...
    #     kwargs.setdefault('batch_size', 100)
        ...
        if dataset == "PaviaU":
            ...
            embed_dims = [320, 320, 512, 512]
        layers = [2, 2, 5, 3]
        model = DCTN(layers, img_size=15, in_chans=n_bands, num_classes=n_classes, embed_dims=embed_dims, patch_size=3, transitions=transitions,
        ...
        segment_dim = [8, 8, 4, 4]
        mlp_ratios = [3, 3, 3, 3]
        embed_dims = [352, 352, 256, 256]## for IN 368, for GRSS 256, for PU 168, for KSC 320 for XA 480
...
def train(
    ...
    display_available = display is not None and hasattr(display, 'line')

    net.to(device)
    best_val_acc = 0.0
    patience_counter = 0
    patience = 20  # 连续多少轮验证集准确率不提升则停止训练
    ...
    for e in tqdm(range(1, epoch + 1), desc="Training the network"):
        ...
        for batch_idx, (data, target) in tqdm(
            ...
            if display_iter and iter_ % display_iter == 0 and display_available:
                ...
                if len(val_accuracies) > 0 and display_available:
        ...
        if val_loader is not None:
            ...
            if val_acc > best_val_acc:
                ...
                # 保存最佳模型
                model_name = camel_to_snake(str(net.__class__.__name__))
                dataset_name = data_loader.dataset.name
                model_dir = f"./checkpoints/{model_name}/{dataset_name}/"
                ...
                tqdm.write(f"Saved best model with val acc: {val_acc:.4f}")
            else:
                patience_counter += 1  # 准确率未提升，增加计数器

                # 早停检查
            if patience_counter >= patience:
                tqdm.write(f"Early stopping at epoch {e}, best val acc: {best_val_acc:.4f}")
                break
            metric = -val_acc
        else:
            metric = avg_loss

...
def applyPCA(X, numComponents=75):
    ...
    newX = np.reshape(newX, (numComponents, X.shape[1], X.shape[2]))
...
def get_dataset(dataset_name, target_folder="./", datasets=DATASETS_CONFIG):
    ...
    """
   # palette = {0:(0, 0, 0), 1:(255, 0, 0), 2:(38, 115, 0), 3:(255, 0, 197),
    #                4:(111, 74, 0), 5:(85, 255, 0), 6:(0, 112, 255), 7:(255, 255, 190)}
    palette = None
    ...
    if dataset_name == "PaviaC":
        ...
        # Load the image
        img = open_file(folder + "HSI.mat")["HSI"]
        ...
        img = open_file(folder + "WHU_Hi_HongHu.mat")["WHU_Hi_HongHu"]
        ...
        # Load the image
        img = open_file(folder + "S2.mat")["img"]

        rgb_bands = (5, 3, 4)

        gt = open_file(folder + "S2_gt.mat")["gt"]
...
class HyperX(torch.utils.data.Dataset):
    ...
    def __init__(self, data, gt, **hyperparams):
        ...
        self.data = data
        self.label = gt
        # self.data = padding_image(data, patch_size=[15, 15])
        # self.label = gt
        ...
        # x_pos, y_pos = np.argwhere(mask)
        p = self.patch_size // 2
        self.indices = np.array(
            [
                (x, y)
                for x, y in zip(x_pos, y_pos)
                if x > p-1 and x < data.shape[0] - p and y > p-1 and y < data.shape[1] - p
            ]
        )
    ...
    def __getitem__(self, i):
        ...
        #data = data[0:103, :, :]
        # d, h, w = data_z.shape[:]
        # pad = np.zeros((144-d, h, w))
        # data = np.concatenate((data_z, pad), axis=0)
        # data = np.asarray(np.copy(data), dtype="float32")
        # data = data[11:14, :,:]
        # data, pca = applyPCA(data, numComponents=4)
        # # print(data.shape)
        # data = np.asarray(np.copy(data), dtype="float32")
